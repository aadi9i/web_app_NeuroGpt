...
def compute_band_powers(eegdata, fs):
    ...
    meanGamma1 =  np.mean(PSD[ind_gam1, :], axis=1).flatten()
    # print(np.array(PSD[ind_gam1, :]).shape)
     # Gamma 40-49.5
    ind_gam2 = np.where((f>=40)&(f<=49.5))
    meanGamma2 =  np.mean(PSD[ind_gam2, :], axis=1).flatten()
    # print(np.array(PSD[ind_gam2, :]).shape)
    # print(meanGamma1.shape,meanGamma2.shape, meanTheta1.shape,meanTheta2.shape, meanAlpha1.shape,meanAlpha2.shape,meanBeta1.shape,meanBeta2.shape)
    feature_vector = np.concatenate((meanTheta1,meanTheta2, meanAlpha1,meanAlpha2,
                                     meanBeta1,meanBeta2,meanGamma1,meanGamma2), axis=0)
